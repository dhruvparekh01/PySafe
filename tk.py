...
class Gui:
    ...
    def sel_dir(self):
        Gui.out_dir = filedialog.askdirectory(parent=self.window, mode='rb', title='Choose a file')
    ...
    def browse(self):
        ...
        Gui.plaintext = bitarray(bit_list)
        Gui.file_selected = 1

    def speed_sel(self):
        selected = self.cb.get()
        if selected == 'Slow (Low CPU usage)':
            Gui.no_of_processes = 1
        elif selected == 'Medium (default)':
            Gui.no_of_processes = 2
        elif selected == 'Fast (High CPU usage)':
            Gui.no_of_processes = 3

    def get_data(self):
        self.speed_sel()
        Gui.password = self.t1.get()
        Gui.out_name = self.t2.get()

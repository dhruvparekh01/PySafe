...
def decrypt_DES(ciphertext, k):
    ...
    input_64_bits = functions.hex_to_bin(ciphertext)  #take the input hexadecimal ciphertext and convert it into binary
    cipherkey_64bit = functions.hex_to_bin(k)  #do the same with the cipherkey
    ...
    for i in range(16):  #16 des round involving multiple steps but in reverse order for decryption
        new_left = list(right)
        new_right = functions.xor(left, functions.round_func(right, key[15-i]))  #perform the round function
        left = new_left
        right = new_right
